          ...
          {EXTERNAL_CATS.includes(activeCategory) ? (
            <div className="px-4 pb-24 mt-4">
              {loadingExternal ? (
                ...
                // Mesi disponibili con eventi presenti e non passati
                ...
                const filtered = externalEvents.filter(e => e.date && e.date.slice(0,7) === selMonth);
                return (
                  <>
                    {/* Selettore mese */}
                    <div className="flex gap-2 mb-5 overflow-x-auto no-scrollbar pb-1">
                      {availMonths.map(ym => {
                        const [y, m] = ym.split('-');
                        const label = IT_MONTHS_LABEL[parseInt(m) - 1];
                        return (
                          <button key={ym} onClick={() => setSagreMonth(ym)}
                        ...
                        {filtered.map(ev => (
                          <a key={ev.id} href={ev.externalUrl} target="_blank" rel="noopener noreferrer"
                            ...
                            {ev.imageUrl
                              ...
                              : <div className="w-full aspect-[3/4] bg-zinc-800 flex items-center justify-center"><Church size={48} className="text-zinc-600" /></div>
                            }
                            <div className="p-5">
                              ...
                              {ev.city && <p className="text-zinc-400 text-xs font-black uppercase tracking-widest mt-1 line-clamp-1">{ev.city}</p>}
                            </div>
                          </a>
                        ))}
                      </div>
